...
- fullscreen loading, reload recovery, polling 반영 규칙
...
- 헤더에는 세션 날짜 라벨만 남기고 manual refresh button은 두지 않는다.
- 본문은 **benchmark board grid**로 동작하며, 카테고리 탭 대신 개별 benchmark를 직접 보여준다.
- 보드는 고정 grid paging 대신 **horizontal scroll rail**로 탐색한다.
- `Arena`, `Capability`, `Multimodal` 같은 umbrella label은 전면에 두지 않는다.
- benchmark 제목은 raw id나 내부 taxonomy보다 **human readability**를 우선한다.
- benchmark 제목 아래의 장황한 subtitle, snapshot 설명, `showing X-Y of N` 같은 운영용 보조문구는 숨긴다.
- 화면이 열려 있는 상태에서 로컬 시간이 `12 PM`을 지나면 하루 한 번 자동으로 refresh한다.
...
<div class="flex flex-wrap items-start justify-between gap-3 border-b border-orbit-border pb-3">
  <div class="min-w-0 flex-1">
    <h1 class="font-display text-[1.12rem] font-semibold text-orbit-text">
...
- session label은 날짜만 보여주고 `live scan` 같은 window text는 붙이지 않는다.
- `showing X-Y of N boards` 같은 위치 안내 텍스트는 기본값으로 두지 않는다.
...
<article class="flex min-h-0 flex-col border border-orbit-border bg-orbit-bg">
  <div class="border-b border-orbit-border px-3 py-2.5">
    <h3 class="font-display text-[0.88rem] font-semibold text-orbit-text">
      Code
    </h3>
  </div>
</article>
...
- 보드 title 아래의 subtitle, snapshot 설명, 과도한 count strip은 기본적으로 노출하지 않는다.

### Rail Density

- 보드는 wrap grid가 아니라 고정 폭 card를 가진 horizontal rail로 배치한다.
- desktop에서는 2~3개 card가 동시에 보이도록 폭을 잡고, 나머지는 옆으로 스크롤한다.
- mobile/tablet에서도 같은 rail 패턴을 유지하되 card 폭만 줄인다.
- 각 보드는 독립 스크롤을 허용하고 ranking은 상위 10개까지 보이도록 유지한다.

## Benchmark Entry Card

정보 행 단위 컴포넌트. 현재 leaderboard entry의 canonical 구현은 **rank + model name + organization + human score**를 빠르게 읽는 데 집중한다.

### 구조

```
┌─────────────────────────────────────────────┐
│ rank │ model name              human score  │
│      │ organization                         │
└─────────────────────────────────────────────┘
```
  ...
  <div class="flex min-w-0 items-center gap-0">
    ...
    <div class="orbit-leaderboard-entry__body min-w-0 flex-1 border-l border-orbit-border px-2 py-2">
      <div class="flex min-w-0 items-baseline justify-between gap-2">
        ...
        <span class="font-mono text-[0.58rem] tabular-nums text-orbit-accent">1,548</span>
      </div>
...
- rank는 작은 mono 숫자로 처리하고 시선은 이름에 먼저 가야 한다.
- organization은 muted uppercase로 보조하고, human score는 정수형 숫자로 보여준다.
...
현재 benchmark entry의 canonical 표현은 **우측 정렬된 정수형 human score**다.

규칙:
- score는 소수점을 제거하고 정수 단위로 반올림해 보여준다.
- score는 `tabular-nums + orbit-accent` 조합으로 고정한다.
- ranking 리스트는 상위 10개까지 유지한다.
- 값이 없으면 숫자 영역을 비우고 이름/organization만 유지한다.
...
- auto refresh 또는 bootstrap reload가 실행 중일 때
...
backend가 내려주는 `loading.steps`는 현재 9단계다.
...
현재 frontend는 Redis-backed job progress를 polling한다.

### Active Job Lookup

- `/api/jobs/active?surface=dashboard`
- homepage bootstrap 또는 manual reload가 진행 중인지 먼저 확인한다.

### Progress Polling

- `/api/jobs/{job_id}`
- `1.5s` 간격으로 진행률, 현재 work item, active source 목록을 갱신한다.

### Refresh Recovery

- 새로고침 시에는 먼저 `GET /api/jobs/active?surface=dashboard`를 읽어 ongoing job이 있는지 확인한다.
- 진행 중이면 일반 화면으로 바로 돌아가지 않고 fullscreen loading을 복구한다.
- job이 없으면 기존 dashboard를 다시 읽는다.
...
| File | Role |
|------|------|
| `src/App.tsx` | fullscreen loading, polling, main dashboard, settings modal |
| `src/index.css` | tokens, grid/scanline, loader styling, hacker reveal, orbit-leaderboard-entry |
| `src/components/dashboard/LeaderboardPanel.tsx` | horizontal benchmark rail, board card, compact entry card, integer human score — canonical 구현체 |
...
9. benchmark board가 horizontal rail로 이어지고 wrap grid/paging button 없이 옆으로 탐색 가능한가
10. `Arena`, `Capability`, `Multimodal` 같은 상위 그룹 탭 없이 보드가 직접 노출되는가
11. `showing X-Y of N boards` 같은 위치 안내 텍스트가 없는가
12. 보드 타이틀이 raw id 대신 읽기 쉬운 이름으로 정리되고 title 아래 군더더기 subtitle이 없는가
13. 각 board card가 현재와 비슷한 시각 밀도를 유지하면서 2~3장 정도가 한 번에 보이는가
14. 각 board에서 ranking이 상위 10개까지 유지되는가
15. 각 entry card에 rank, 이름, organization, integer human score 순서가 유지되는가
16. 값이 없는 항목에서도 entry card 레이아웃이 무너지지 않는가
17. session label에 `live scan` 같은 window text가 보이지 않는가
18. 앱이 켜진 상태에서 로컬 시간 `12 PM`을 넘기면 하루 한 번 자동 refresh가 시작되는가
