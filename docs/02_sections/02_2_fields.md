[Index](../README.md) · [🇰🇷 한국어](./02_2_fields.ko.md) · [01. Overall Flow](../01_overall_flow.md) · [02. Sections](./README.md) · [02.1 Sources](./02_1_sources.md) · **02.2 Fields** · [03. Runtime Flow](../03_runtime_flow.md) · [04. LLM Usage](../04_llm_usage.md) · [05. Data Collection Pipeline](../05_data_collection_pipeline.md) · [06. UI Design Guide](../06_ui_design_guide.md)
...
<!-- ────────────────────────────────────────────
     ...
     6      reference 블록
     7      tags 체계
     8      LLM 판정/분류 결과 (company filter, paper domain)
     9      소스별 필드 커버리지 매트릭스
     ──────────────────────────────────────────── -->
...
| `models` | HF 모델 카드 | hf_trending_models |
| `community` | 커뮤니티/개발자 | hn_topstories, reddit_localllama, reddit_machinelearning, github_curated_repos |
| `company` | 글로벌 기업 | openai_news_rss, google_ai_blog, microsoft_research, nvidia_press_releases, nvidia_research_ai, apple_ml, amazon_science, hf_blog, anthropic_news, deepmind_blog, groq_newsroom, mistral_news, stability_news, salesforce_ai_research_rss |
| `company_kr` | 한국 기업 | samsung_research_posts, kakao_tech_rss, naver_cloud_blog_rss |
...
| 값 | 의미 | 해당 소스 |
|----|------|-----------|
| `paper` | 학술 논문/프리프린트 | arxiv_rss_cs_ai, arxiv_rss_cs_lg, hf_daily_papers, nvidia_research_ai |
| `blog` | 블로그/기술 포스트 | openai_news_rss, google_ai_blog, microsoft_research, apple_ml, amazon_science, hf_blog, qwen_blog_rss, salesforce_ai_research_rss, kakao_tech_rss, naver_cloud_blog_rss, samsung_research_posts |
| `news` | 뉴스/발표 | anthropic_news, mistral_news, nvidia_press_releases, stability_news, groq_newsroom |
...
| `metadata_only` | 본문 없음, 메타데이터만 | hf_trending_models, github_*_repos |
...
- 기업 블로그 대부분 (OpenAI, Google AI, NVIDIA, Apple ML, Amazon Science, Anthropic, DeepMind, Mistral, Stability, Groq, Salesforce, Qwen, DeepSeek, Kakao, NAVER, Samsung)
- HF Blog
...
| `primary_reason` | 점수가 높아진 주된 이유 (`trending_feed`, `fresh_release`, `established` 등) |
...
| `external_ids.hf_model_id` | hf_trending_models | `deepseek-ai/DeepSeek-R1` |
...
| `nvidia_press_releases` | `https://nvidianews.nvidia.com/cats/press_release.xml` | `https://nvidianews.nvidia.com/news/{slug}` |
| `nvidia_research_ai` | `https://research.nvidia.com/research-area/machine-learning-artificial-intelligence` | `https://research.nvidia.com/publication/{slug}` |
...
<!-- ────────────────────────────────────────────
     ...
     실제 LLM 판정/분류 결과는 별도 NDJSON 파일에 저장된다.
...
### 8.2 현재 구현된 LLM 출력 (별도 파일)

LLM 결과는 `documents.ndjson`을 수정하지 않고, `labels/` 디렉토리에 별도 NDJSON로 저장된다.
