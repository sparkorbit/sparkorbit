...
기본 동작은 source별 기본 limit를 사용한다. 일반 source는 `20개`, paper 계열 source는 source당 `16개`, `hn_topstories` 같은 저밀도 source는 더 낮게 가져온다. `--limit`을 주면 모든 source에 동일한 override가 적용된다.
