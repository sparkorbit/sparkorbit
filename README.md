...
- `PoC/llm_enrich`: local LLM enrichment tooling

canonical source of truth는 여전히 `PoC/source_fetch/data/runs/<run_id>/` 아래의 JSONL/JSON run output이다. Redis는 현재 세션을 빠르게 서빙하기 위한 materialized session layer이고, frontend는 backend API/BFF만 사용한다.
...
- [docs/04_llm_usage.md](./docs/04_llm_usage.md)
  offline enrichment와 session summary provider의 현재 범위
...
- backend와 frontend는 현재 저장소 안에 구현되어 있고, `docker compose up`으로 로컬에서 함께 띄울 수 있다.
- homepage 진입 시 active session이 없으면 backend가 실제 collection을 시작하고, frontend는 SSE fullscreen loader로 진행 단계를 보여준다.
- `PoC/llm_enrich`는 별도 오프라인 enrichment tooling이고, homepage summary/digest는 backend session runtime이 담당한다.
...
- local LLM tooling entrypoint는 [PoC/llm_enrich/scripts/llm_enrich.py](./PoC/llm_enrich/scripts/llm_enrich.py), [PoC/llm_enrich/scripts/paper_enrich.py](./PoC/llm_enrich/scripts/paper_enrich.py) 다.
...
cd sparkorbit/PoC/source_fetch
python3 -m venv .venv
. .venv/bin/activate
...
- worker 컨테이너는 queue 기반 enrichment를 위한 sidecar이고, homepage bootstrap과 reload는 현재 API background task 경로에서도 끝까지 진행된다.
