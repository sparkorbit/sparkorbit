...
Node-based frontend tooling (`npm install`, `npm run build`, `npm run dev`) requires Node `^20.19.0 || >=22.12.0`. Docker builds already use `node:22`, so the full stack path works the same on Linux, macOS, and Windows as long as Docker Desktop/Engine is installed.
...
macOS/Windows에서 Bash 진입점이 불편하면 아래처럼 `docker compose`를 직접 실행하면 된다.
